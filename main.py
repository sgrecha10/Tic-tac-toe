...
class Field:
    ...
    def draw_field(self):
        ...
        root.geometry(str(WIDTH) + str('x') + str(HEIGHT))
        ...
        for i in range(SIZE):
            for k in range(SIZE):
                ...
                self.c.tag_bind(wid, "<Button-1>", lambda event, i=i, k=k: self.mark_cell(i, k, 'human'))
                tmp.append((i, k))
        self.arrField = dict.fromkeys(tmp)

    def mark_cell(self, i, k, player):
        # self.c.delete(wid)
        if player == 'human':
            form = HUMAN
        else:
...
class Ai:
    def step(self, i, k):
        ...
        i, k = random.choice([key for key in a.field.arrField if a.field.arrField[key] is None])
        # перечеркиваем выигравшие клеточки
        a.field.draw_line((0, 1), (2, 1))
        # ai ходит
        a.field.mark_cell(i, k, 'bot')
